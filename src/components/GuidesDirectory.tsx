    ...
    <section id="all-guides" className="mt-10">
      <div className="border-b border-[#dfe4e5] pb-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-semibold text-[#7a8182]">全部指南</p>
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          <label className="relative block">
            ...
            <input
              ...
              className="h-11 w-full rounded-md bg-[#f9f9f9] pl-11 pr-11 text-sm text-[#2d3435] outline-none ring-1 ring-[#adb3b4]/25 transition placeholder:text-[#8b9293] focus:bg-white focus:ring-[#45bf78]/45"
            />
            {query ? (
              <button
                type="button"
                onClick={() => setQuery("")}
                className="absolute right-3 top-1/2 inline-flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-md text-[#5a6061] transition hover:bg-[#e4e9ea] hover:text-[#202829]"
                aria-label="清空搜索"
              >
                <X size={15} />
              </button>
      ...
      <div>
        <div className="flex items-center justify-between gap-3 border-b border-[#dfe4e5] py-4">
          <div>
          ...
          {(query || activeCategory !== "all") ? (
            <button
              ...
              className="inline-flex h-9 shrink-0 items-center rounded-md bg-[#edf0f1] px-3 text-sm font-semibold text-[#2d3435] transition hover:bg-[#dde4e5]"
            >
              重置
            </button>
          ) : null}
        ...
        {filteredGuides.length ? (
            ...
            {filteredGuides.map((guide) => {
              ...
              return (
                <Link
                  key={guide.href}
                  href={guide.href}
                  className="group grid gap-4 py-5 transition hover:bg-[#edf0f1]/60 sm:px-2 xl:grid-cols-[minmax(0,1fr)_136px]"
                >
                  <span>
                  ...
                  <span className="flex items-center justify-start xl:justify-end">
                    <span className="inline-flex h-9 items-center gap-2 rounded-md bg-[#2d3435] px-3 text-sm font-semibold text-[#f8f8f8] transition group-hover:bg-[#202829]">
                      打开指南
          ...
          <div className="py-14 text-center">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-md bg-[#edf0f1] text-[#5a6061]">
            ...
            <button
              ...
              className="mt-5 inline-flex h-10 items-center rounded-md bg-[#dde4e5] px-4 text-sm font-semibold text-[#2d3435] transition hover:bg-[#d3dcdd]"
            >
              查看全部指南
            </button>
          </div>
...
function CategoryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex h-9 shrink-0 items-center rounded-md px-3 transition ${
        active
          ? "bg-[#2d3435] text-[#f8f8f8]"
          : "bg-[#edf0f1] text-[#5a6061] hover:bg-[#dde4e5] hover:text-[#202829]"
      }`}
    >
      {label}
    </button>
