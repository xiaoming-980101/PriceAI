...
如果你是 AI 周边服务、开发者工具、API Gateway、中转 API、云服务、网络检测、支付或账号安全相关服务方，可以查看 [商务合作](https://priceai.cc/commercial)。
...
商业部署、品牌合作、赞助位、认证收录、数据合作或其他授权需求，请查看 [商务合作](https://priceai.cc/commercial)。

`PriceAI` 名称、Logo、域名、视觉品牌、线上生产数据、渠道数据、价格快照、指南内容、截图和公开素材不随源码一起授权。Fork、二次开发或公开展示时，请阅读 [数据与内容授权](./DATA_LICENSE.md) 和 [品牌与商标政策](./TRADEMARKS.md)，并避免让用户误认为你的服务是官方 PriceAI。
